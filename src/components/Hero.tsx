...
export default function Hero({ onSignUp }: { onSignUp: () => void }) {
    return (
        <section className="relative min-h-screen bg-gradient-to-br from-[#F0F9FF] via-white to-[#F0FDFA] overflow-hidden flex items-center pt-20">
            {/* BG decoration */}
            <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-sky-100/50 rounded-full -translate-y-1/2 translate-x-1/4 blur-3xl" />
            <div className="absolute bottom-0 left-0 w-[400px] h-[400px] bg-teal-100/40 rounded-full translate-y-1/3 -translate-x-1/4 blur-3xl" />
            <div className="absolute top-1/3 left-1/3 w-64 h-64 bg-violet-100/30 rounded-full blur-2xl" />
            ...
            <div className="relative max-w-7xl mx-auto px-6 py-24 grid lg:grid-cols-2 gap-16 items-center">
                ...
                >
                    <FadeUp>
                        <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-sky-100 text-sky-600 text-sm font-semibold w-fit">
                    ...
                    <FadeUp delay={0.3} className="flex flex-wrap gap-4">
                        <button onClick={onSignUp} className="group flex items-center gap-2 px-8 py-4 rounded-full bg-gradient-to-r from-sky-500 to-teal-500 text-white font-semibold shadow-xl shadow-sky-200 hover:shadow-sky-300 hover:scale-105 transition-all">
                            Get Started
                            <ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" />
                        </button>
                        <button className="flex items-center gap-2 px-8 py-4 rounded-full bg-white border-2 border-slate-200 text-slate-700 font-semibold hover:border-sky-300 hover:shadow-md transition-all">
                ...
                >
                    ...
                    {/* Floating badges */}
                    {[
                        { label: "AI Powered", color: "bg-violet-500", pos: "left-0 top-1/4" },
                        { label: "Instant Booking", color: "bg-teal-500", pos: "right-0 top-1/3" },
                        { label: "500+ Hospitals", color: "bg-sky-500", pos: "left-4 bottom-1/4" },
                    ].map(b => (
                        <motion.div
                            key={b.label}
                            animate={{ y: [0, -8, 0] }}
